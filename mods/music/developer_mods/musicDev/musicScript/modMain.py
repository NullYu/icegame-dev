...
@Mod.Binding(name="music", version="0.1")
class LobbyServerMod(object):
    ...
    def initServer(self):
        # 打印服务端info log
        logger.info('===========================init music system mod!===============================')
        # 函数可以将System注册到服务端引擎中，实例的创建和销毁交给引擎处理。第一个参数是名字空间，第二个是System名称，第三个是自定义MOD System类的路径
        # 取名名称尽量个性化，不能与其他人的MOD冲突，可以使用英文、拼音、下划线这三种。
        self.lobbyServer = serverApi.RegisterSystem("music", "musicSystem",
                                                    "musicScript.musicServerSys.musicServerSystem")
