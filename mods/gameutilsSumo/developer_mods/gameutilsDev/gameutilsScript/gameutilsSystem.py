...
class gameutilsServerSys(ServerSystem):
    ...
    def ListenEvents(self):
        ...
        def b():
            ...
            print 'send sumo overide'
            serverId = lobbyGameApi.GetServerId()
            self.RequestToService("matchmaking", "RecordSidEvent", args)
    ...
    def OnScriptTickServer(self):
        mainSystem = serverApi.GetSystem('unrankedGame', 'unrankedGameSystem')
        if mainSystem.gameTime >= 0 and not mainSystem.peace:
            for player in serverApi.GetPlayerList():
                comp = serverApi.GetEngineCompFactory().CreatePos(player)
                yPos = comp.GetFootPos()[1]
                if yPos < 195:
                    li = serverApi.GetPlayerList()
                    li = li.pop(li.index(player))
                    mainSystem.OnPlayerDie({
                        'id': player,
                        'attacker': li
                    })
