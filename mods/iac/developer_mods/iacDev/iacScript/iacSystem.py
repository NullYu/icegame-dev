...
class iacSystemSys(ServerSystem):
    ...
    def ListenEvents(self):
        ...
        self.ListenForEvent('iacBeh', 'iacClient', 'CaughtModpcEvent', self, self.OnCaughtModpc)
    ...
    def OnAddServerPlayer(self, data):
        ...
        # launcher.invalidUsername
        isOk = commonNetgameApi.CheckNameValid(lobbyGameApi.GetPlayerNickname(playerId))
        if not isOk:
            lobbyGameApi.TryToKickoutPlayer(playerId, "§e§l无法登录\n§r这不是一个惩罚。\n§l为什么？§r您的用户名不合法，或有错误，因此不能登录。")
    ...
    # launcher.invalidPlatform
    def OnCaughtModpc(self, playerId):
        conf = lobbyGameApi.GetCommonConfig()
        if not conf['review_stage']:
            print 'test environment, allowing modpc!!!'
            return

        uid = lobbyGameApi.GetPlayerUid(playerId)
        sql = 'SELECT * FROM items WHERE uid=%s AND type="whitelist" AND (expire<0 OR expire>%s);'

        def Cb(args):
            if not args:
                lobbyGameApi.TryToKickoutPlayer(playerId,
                                                "no admission to modpc\nEmail service@icegame.xyz for whitelisting\n\nYour UID: %s" % uid)

        mysqlPool.AsyncQueryWithOrderKey('acm70sd18723', sql, (uid, time.time()), Cb)
    ...
    def OnPlayerAttackEntity(self, data):
        ...
        if dist >= 7:
            # self.AddVl(lobbyGameApi.GetPlayerUid(playerId), 4)
